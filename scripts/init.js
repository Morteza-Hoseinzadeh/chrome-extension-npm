...
function getExtensionFiles(framework, useTs, type) {
  ...
  if (type === "popup" || type === "both") {
    files["popup.html"] =
      `<html><body><h1>${framework} Extension Popup</h1></body></html>`;
  }

  if (type === "background" || type === "both") {
    files[`background.${ext}`] =
      `// ${framework} background logic (${useTs ? "TypeScript" : "JavaScript"})`;
  }
...
async function init() {
  ...
  const targetDir = path.join(process.cwd(), "zephyr-extension");

  if (fs.existsSync(targetDir)) {
    console.log(
      `⚠️ Directory ${targetDir} already exists. Please remove or choose a different location.`
    );
    process.exit(1);
  }
  ...
  console.log(
    `✅ ${framework} extension scaffolded successfully in ${targetDir}`
  );
}
